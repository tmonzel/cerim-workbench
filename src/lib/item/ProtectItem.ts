...
import type { ArmorEntity } from './entities';
import { Item } from './Item';
import type { ArmorWeightClass } from './types';
...
export class ProtectItem extends Item {
	...
	constructor(id: string, entity: ArmorEntity) {
		...
		this.resistanceAvg =
			Math.round(((this.resistance.immunity + this.resistance.robustness + this.resistance.focus + this.resistance.vitality) / 4) * 10) / 10;
		...
		this.damageNegationAvg = Math.round((damageNegationValues.reduce((p, c) => p + c, 0) / damageNegationValues.length) * 10) / 10;

		this.weightClass = entity.weightClass;

		if (entity.config && entity.config.effects) {
			this.applyEffects(Object.values(entity.config.effects));
		}
	}
}
