<script lang="ts">
	...
	import type { NumericStat } from '$lib/core';

  const modifiedStats = derived(heroState, (hero) => Object.values(hero.stats).filter(s => s.value.modified && s.value instanceof DynamicNumber) as NumericStat[]);
</script>
