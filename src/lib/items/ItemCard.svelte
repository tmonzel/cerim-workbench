    ...
    <div class="flex items-center gap-x-12 mt-3 mb-2">
      {#if item.type === 'weapon'}
        <p class="text-xl flex gap-2">
          <ValueBadge value={item.damage.getTotal()} isModified={item.damage.isModified()} />
        </p>
      {/if}
      ...
      {#if item.type === 'armor'}
        <p class="text-xl font-light flex items-center gap-x-1">
          ...
          <ValueBadge value={item.armor.getTotal()} isModified={item.armor.isModified()} />
        </p>
      {/if}
      ...
      <p class="text-xl font-light flex items-center gap-x-1">
        ...
        <ValueBadge value={item.weight.getTotal()} isModified={item.weight.isModified()} />
      </p>
    </div>
