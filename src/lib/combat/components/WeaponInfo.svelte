<script lang="ts">
	...
	$: scalingFlags = weapon.getScalingFlags();
</script>

<div class="flex gap-x-8 text-sm">
	{#if weapon.attributeRequirements}
		{@const invalidAttributes = validateRequirements(weapon.attributeRequirements, $heroState.totalAttributes)}
		<div class="flex">
			<svg
				class="me-2"
			...
			>
			<div>
				{#each list(weapon.attributeRequirements) as attr}
		...
		<div>
			<div class="flex items-center">
				<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#e4e4e7"
					...
					/></svg
				>
				<span class="ml-1 text-xs font-semibold">{weapon.attackSpeed}</span>
			</div>
		</div>
	{/if}
	...
	<div>
		...
		<span>{weapon.attackInfo.damage.map((t) => damageTypeRecord[t].name).join('/')}</span>
	</div>
	<div>
		<span class="text-zinc-500">Critical</span><br />
		<span>{weapon.attackInfo.crit}</span>
	</div>

	{#if weapon.statusEffects}
		<div>
			<div class="text-zinc-500 mb-1">Effects</div>
			<div class="flex gap-4">
				{#each list(weapon.statusEffects) as se}
					<div class="flex items-center">
