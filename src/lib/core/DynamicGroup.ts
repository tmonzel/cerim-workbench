...
export class DynamicGroup<T extends string = string> implements DynamicGroupInterface<T> {
	...
	multiply(value: Partial<Record<T, number>>): void {
		for (const k in this._value) {
			if (!value[k]) {
				continue;
			}

			this._value[k].multiply(value[k]);
		}
	}
	...
	addTotalMultiplier(value: Partial<Record<T, number>>): void {
		for (const k in this._value) {
			if (!value[k]) {
				continue;
			}

			this._value[k].addTotalMultiplier(value[k]! - 1);
		}
	}
	...
	getModifiedValues(): { key: T; value: DynamicNumber }[] {
		return Object.entries<DynamicNumber>(this._value)
			.map(([key, value]) => ({ key: key as T, value }))
			.filter((m) => m.value.modified);
	}
